...
# test cases done insomuch
  ...
  $scope.openFixUploadIssues = (data) ->
    ...
    $scope.modal.modalInstance = $uibModal.open(
      templateUrl: 'http://1.'+location.host+'/public/webapp/Globals/modals/fixUploadIssueModal.html',
      size: "lg",
      backdrop: 'static',
      scope: $scope
    )
  ...
  $scope.updateTax = (item) ->
    ...
    if $scope.taxValueUpdated
      ...
      $scope.updateTax.modalInstance = $uibModal.open(
        templateUrl: 'http://1.'+location.host+'/public/webapp/Globals/modals/update-tax.html'
        size: "md"
        backdrop: 'static'
        scope: $scope
      )
  ...
  $scope.addSiteAccountSuccess = (res) ->
    ...
    if res.body.mfa
      ...
      $scope.banks.modalInstance = $uibModal.open(
        templateUrl: 'http://1.'+location.host+'/public/webapp/Globals/modals/yodleeMfaModal.html'
        size: "sm"
        backdrop: 'static'
        scope: $scope
      )
...
#    userServices.getAccounts(companyUniqueName).then($scope.getAccountsSuccess, $scope.getAccountsFailure)
  ...
  $scope.connectBankSuccess = (res) ->
    ...
    modalInstance = $uibModal.open(
      templateUrl: 'http://1.'+location.host+'/public/webapp/Globals/modals/connectBankModal.html',
      size: "md",
      backdrop: 'static',
      scope: $scope,
  ...
  $scope.reconnectAccountSuccess= (res) ->
    ...
    $scope.connectUrl = url
    modalInstance = $uibModal.open(
      templateUrl: 'http://1.'+location.host+'/public/webapp/Globals/modals/refreshBankAccountsModal.html',
      size: "md",
      backdrop: 'static',
      scope: $scope
    )
